...
public class JGrassMapGeoResource extends IGeoResource {
    ...
    public JGrassMapGeoResource( IService parentService, JGrassMapsetGeoResource parentMapset,
            String mapName, String mapTypeAndPath ) {
    ...
    public <T> boolean canResolve( Class<T> adaptee ) {
        ...
        return adaptee.isAssignableFrom(IService.class)
                ...
                || adaptee.isAssignableFrom(JGrassMapGeoResource.class)
        ...
        public JGrassMapGeoResourceInfo( IProgressMonitor monitor ) {
            ...
            try {
                 ...
                 */
                    ...
                    String grassasciiFilePath = mapsetPath + File.separator
                            + JGrassConstants.GRASSASCIIRASTER + File.separator + this.name;

                    fileWindow = getGrassAsciiFileWindow(grassasciiFilePath);
                    bounds = new ReferencedEnvelope(fileWindow.getEnvelope(),
                            ((JGrassMapsetGeoResource) JGrassMapGeoResource.this.parent)
                                    .getJGrassCrs());

                    super.icon = AbstractUIPlugin.imageDescriptorFromPlugin(JGrassPlugin.PLUGIN_ID,
                            "icons/obj16/grassascii.gif"); //$NON-NLS-1$
                }
                /*
                 * what if it is an esri ascii grid map?
                 */
                else if (JGrassMapGeoResource.this.type.equals(JGrassConstants.ESRIRASTERMAP)) {
                    String esriiFilePath = mapsetPath + File.separator
                            + JGrassConstants.ESRIASCIIRASTER + File.separator + this.name;
                    ...
                    super.icon = AbstractUIPlugin.imageDescriptorFromPlugin(JGrassPlugin.PLUGIN_ID,
                            "icons/obj16/esrigrid.gif"); //$NON-NLS-1$
                }
                /*
                 * what if it is a fluidturtle ascii raster map?
                 */
                else if (JGrassMapGeoResource.this.type.equals(JGrassConstants.FTRASTERMAP)) {
                    String fluidturtleFilePath = mapsetPath + File.separator
                            + JGrassConstants.FLUIDTURTLEASCIIRASTER + File.separator + this.name;
                    ...
                    super.icon = AbstractUIPlugin.imageDescriptorFromPlugin(JGrassPlugin.PLUGIN_ID,
                            "icons/obj16/ftraster.gif"); //$NON-NLS-1$
                }
                 ...
                 */
                    ...
                    File sitesFile = new File(jGrassMapEnvironment.getMAPSET(),
                            JGrassConstants.SITE_LISTS + "/" + jGrassMapEnvironment.getMapName());
                ...
                super.icon = AbstractUIPlugin.imageDescriptorFromPlugin(JGrassPlugin.PLUGIN_ID,
                        "icons/obj16/problem.gif"); //$NON-NLS-1$
                e.printStackTrace();
            }
    ...
    public RasterData getDataInRegion( JGrassRegion region, IProgressMonitorJGrass monitor )
        ...
        JGrassRasterMapReader jgrassMapReader = new JGrassRasterMapReader.BuilderFromMapPath(
                region, jGrassMapEnvironment.getMapFile().getAbsolutePath()).maptype(
                JGrassConstants.GRASSBINARYRASTERMAP).monitor(monitor).build();
    ...
    private JGrassRegion getGrassAsciiFileWindow( String filepath ) {
        ...
        try {
            ...
            if (fileMapHeader.containsKey(JGrassConstants.HEADER_NS_RES)) {
                fileWindow = new JGrassRegion(Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_WEST)), Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_EAST)), Double.parseDouble(fileMapHeader
    ...
    private JGrassRegion getEsriGridFileWindow( String filepath ) {
        ...
        try {
            ...
            xres = yres = Double.parseDouble(fileMapHeader
                    .get(JGrassConstants.ESRI_HEADER_CELLSIZE));
    ...
    private JGrassRegion getFluidturtleFileWindow( String filepath ) {
        ...
        try {
                                ...
                                if ((line = fluidasciireader.readLine().toLowerCase().trim()) != null) {
                                    if (line.trim().startsWith((i + 1) + ":")) {
                                        ...
                                        // defines the data matrix rows and cols
                                        {
                                            ...
                                            fileMapHeader.put(JGrassConstants.HEADER_ROWS,
                                                    rowsandcols[0].trim());
                                            fileMapHeader.put(JGrassConstants.HEADER_COLS,
                                                    rowsandcols[1].trim());
                                            break;
                                        } else {
                                            ...
                                            fileMapHeader.put(JGrassConstants.HEADER_EW_RES,
                                                    values[0].trim());
                                            fileMapHeader.put(JGrassConstants.HEADER_NS_RES,
                                                    values[1].trim());
                                            fileMapHeader.put(JGrassConstants.HEADER_SOUTH,
                                                    values[2].trim());
                                            fileMapHeader.put(JGrassConstants.HEADER_WEST,
                                                    values[3].trim());
                                            break;
                                        }
            ...
            if (fileMapHeader.containsKey(JGrassConstants.HEADER_NS_RES)) {
                fileWindow = new JGrassRegion(Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_WEST)), east, Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_SOUTH)), north, Double
                        .parseDouble(fileMapHeader.get(JGrassConstants.HEADER_EW_RES)), Double
                        .parseDouble(fileMapHeader.get(JGrassConstants.HEADER_NS_RES)));
            } else if (fileMapHeader.containsKey(JGrassConstants.HEADER_COLS)) {
                fileWindow = new JGrassRegion(Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_WEST)), east, Double.parseDouble(fileMapHeader
                        .get(JGrassConstants.HEADER_SOUTH)), north, Integer.parseInt(fileMapHeader
                        .get(JGrassConstants.HEADER_ROWS)), Integer.parseInt(fileMapHeader
                        .get(JGrassConstants.HEADER_COLS)));
            } else {
                fileWindow = null;
