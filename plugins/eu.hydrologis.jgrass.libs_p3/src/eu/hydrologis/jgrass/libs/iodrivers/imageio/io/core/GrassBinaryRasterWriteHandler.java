...
public class GrassBinaryRasterWriteHandler {
    ...
    public JGrassRegion getWriteRegion() throws IOException {
        JGrassRegion wind = writerGrassEnv.getActiveRegion();
        return wind;
    }
    ...
    private void createUtilityFiles( JGrassRegion dataRegion ) throws IOException {
        ...
        JGrassRegion tmp = new JGrassRegion(writerGrassEnv.getWIND().getAbsolutePath());
        ...
        // hist/<name>
        OutputStreamWriter historyFile = new OutputStreamWriter(new FileOutputStream(writerGrassEnv
                .getHIST()));
        Date date = new Date();
        historyFile.write(date + "\n"); //$NON-NLS-1$
        historyFile.write(writerGrassEnv.getCELL().getName() + "\n"); //$NON-NLS-1$
        historyFile.write(writerGrassEnv.getMAPSET().getAbsolutePath() + "\n"); //$NON-NLS-1$
        historyFile.write(System.getProperty("user.name") + "\n"); //$NON-NLS-1$ //$NON-NLS-2$
        historyFile.write("DCELL\n"); //$NON-NLS-1$
    ...
    private void createCellhd( int chproj, int chzone, double chn, double chs, double che,
        ...
        OutputStreamWriter cellHeadFile = new OutputStreamWriter(new FileOutputStream(writerGrassEnv
                .getCELLHD()));
        cellHeadFile.write(data.toString());
        cellHeadFile.close();
    }
